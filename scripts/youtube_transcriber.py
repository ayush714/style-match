...
class YoutubeTranscriber(object):
    ...
    def submit(self):
        if self.upload_url is not None:
            ...
            print(f"A transcription job (id={self.transcription_id}) has been submitted")

        else:
            raise ValueError("upload_url has not been set yet. Please upload an audio file")

    def poll(self):
        if self.transcription_id is not None:
            endpoint = f"https://api.assemblyai.com/v2/transcript/{self.transcription_id}"
            headers = {
                "authorization": self.api_key,
            }
            status = ""
